...
#include "ch.h"
#include "m4can.h"
#include "m4radio_status.h"
//#include "m4radio_router_slots.h"
#include "m4radio_router.h"
//#include "m4radio_labrador.h"
 ...
 */
void m4radio_router_handle_can(uint16_t sid, bool rtr,
                               uint8_t* data, uint8_t dlc)
{
//    if(sid >= 2048) {
//        m4status_set_error(M4RADIO_COMPONENT_ROUTER,
//                           M4RADIO_ERROR_ROUTER_BAD_MSGID);
//        return;
//    }
//
//    struct m4radio_slot* slot = &m4radio_slots[sid];
//
//    if(slot->mode == M4RADIO_ROUTER_MODE_NEVER)
//    {
//        return;
//    } else if(slot->mode == M4RADIO_ROUTER_MODE_ALWAYS)
//    {
//        enqueue(sid, rtr, data, dlc);
//    } else if(slot->mode == M4RADIO_ROUTER_MODE_TIMED)
//    {
//        if(chVTTimeElapsedSinceX(slot->tx_time) > MS2ST(slot->period)) {
//            slot->tx_time = chVTGetSystemTimeX();
//            enqueue(sid, rtr, data, dlc);
//        }
//    } else if(slot->mode == M4RADIO_ROUTER_MODE_COUNT)
//    {
//        slot->skip_count++;
...
//    }
//
//    if(sid != (CAN_ID_M4RADIO|CAN_MSG_ID_STATUS)) {
//        m4status_set_ok(M4RADIO_COMPONENT_ROUTER);
//    }
}
...
void m4radio_router_fillbuf(uint8_t* buf, size_t len)
{
    msg_t mailbox_rv;
    msg_t msg;
...
void m4radio_router_init() {
    m4status_set_init(M4RADIO_COMPONENT_ROUTER);
